...
function GuardianCard({ guardian, pet }: { guardian: GuardianView; pet: Pet }): JSX.Element {
  ...
  return <article className={`guardian-card ${isOwner ? 'guardian-card-owner' : ''}`}>
    <div className="guardian-card-top"><PetAvatar pet={pet} size="small" showPresetLabel={false} /><div><p className="eyebrow">{isOwner ? '主监护人' : '共同监护人'}</p><h3>{name}</h3><p className="muted-text">{isOwner ? '负责家庭邀请与资料维护' : '已接受共同守护'}</p></div><StatusTag tone="success">已加入</StatusTag></div>
...
export default function FamilyPage({ petId }: FamilyPageProps): JSX.Element {
  ...
  async function submitInvite(): Promise<void> {
    ...
    try {
      ...
      showSuccess('邀请已生成，请把链接或二维码发给家人');
    } catch (error) {
  ...
  /** 在当前浏览器打开真实 Hash 邀请地址，模拟受邀者进入公开页。 */
  ...
  async function decideParentRelation(relation: Relation, status: 'accepted' | 'rejected'): Promise<void> {
    try {
      ...
      showSuccess(status === 'accepted' ? '对方已确认亲缘关系' : '对方已拒绝亲缘关系');
    } catch (error) {
  ...
  const roleDescription = state.currentRole === 'owner' ? '当前身份：主监护人' : state.currentRole === 'invitee' ? '当前身份：模拟受邀者' : '当前身份：访客（只读）';
  ...
  return <div className="family-page">
    ...
    {familyCelebration && <div className="family-celebration" role="status" aria-live="polite"><span className="ribbon ribbon-one">PAWS</span><span className="ribbon ribbon-two">+50</span><span className="ribbon ribbon-three">一起守护</span><strong>首次共同守护奖励 +50 PAWS（演示）</strong><span>{acceptedInvite?.inviteeNickname ?? '新家人'} 已加入家庭</span></div>}
    ...
    {tab === 'guardians' && <section className="family-panel" role="tabpanel" aria-label="监护家庭">
      ...
      <section className="family-subsection"><div className="family-section-heading"><div><p className="eyebrow">新成员</p><h2>共同守护卡</h2></div></div>{acceptedInvite ? <div className="family-pair-card"><div className="family-pair-avatar"><PetAvatar pet={pet} size="small" showPresetLabel={false} /><span className="pair-plus">＋</span><PetAvatar pet={pet} size="small" showPresetLabel={false} /></div><div><strong>{owner?.user?.nickname ?? '主监护人'} 与 {acceptedInvite.inviteeNickname}</strong><p className="muted-text">一起陪伴 {pet.name} 成长 · {formatDate(acceptedInvite.acceptedAt)}</p><span className="family-reward-copy">首次共同守护奖励 +50 PAWS（演示）</span></div></div> : <EmptyState title="等待共同守护者加入" description="接受邀请后，这里会展示双人宠物卡与加入时间。" />}</section>
    </section>}
    {tab === 'tree' && <section className="family-panel" role="tabpanel" aria-label="宠物族谱"><FamilyGraph state={state} petId={pet.id} siblings={siblings} onSelectPet={setSelectedPet} onAddParent={(role) => { if (canManage) openParentModal(role); }} /><section className="family-subsection relation-subsection"><div className="family-section-heading"><div><p className="eyebrow">关系申请</p><h2>父母关系处理</h2></div><Button variant="secondary" disabled={!canManage} onClick={() => openParentModal()} title={!canManage ? '当前身份没有新增亲缘关系的权限' : '添加父母'}><Plus size={17} aria-hidden="true" />添加父母</Button></div>{!canManage && <p className="family-readonly-hint">当前身份没有监护权限，亲缘资料只读；申请和处理按钮已禁用。</p>}{childRelations.length === 0 ? <p className="muted-text">还没有父母关系记录，可以从图谱中的虚线节点开始添加。</p> : <div className="relation-list">{childRelations.map((relation) => { const relationView = relationStatus(relation.status); const parent = state.pets.find((candidate) => candidate.id === relation.parentPetId); return <article className="relation-record" key={relation.id}><div><strong>{relation.role === 'father' ? '父亲' : '母亲'}：{parent?.name ?? '未知宠物'}</strong><p className="muted-text">{parent ? describePet(parent) : '资料不存在'} · 提交于 {formatDate(relation.createdAt)}</p></div><div className="relation-record-actions"><StatusTag tone={relationView.tone}>{relationView.label}</StatusTag>{relation.status === 'pending' && <><Button variant="secondary" disabled={!canManage} onClick={() => void decideParentRelation(relation, 'accepted')} title={!canManage ? '当前身份没有处理亲缘申请的权限' : '模拟对方接受'}><Check size={15} aria-hidden="true" />模拟接受</Button><Button variant="ghost" disabled={!canManage} onClick={() => void decideParentRelation(relation, 'rejected')} title={!canManage ? '当前身份没有处理亲缘申请的权限' : '模拟对方拒绝'}><X size={15} aria-hidden="true" />模拟拒绝</Button></>}</div></article>; })}</div>}</section></section>}
    <Modal open={inviteOpen} onClose={closeInviteModal} title="邀请家人一起守护"><p className="muted-text">邀请链接只用于本地演示；受邀者打开后可在确认页接受或拒绝。</p><InputField label="受邀者昵称" value={inviteNickname} maxLength={20} placeholder="例如：阿宁" onChange={(event) => setInviteNickname(event.target.value)} error={inviteError} /><div className="button-row"><Button onClick={() => void submitInvite()} disabled={!canInvite || inviteSubmitting}><UserPlus size={16} aria-hidden="true" />{inviteSubmitting ? '生成中…' : '生成邀请'}</Button><Button variant="secondary" onClick={closeInviteModal}>稍后处理</Button></div>{createdInvite && <div className="invite-created"><div className="invite-created-heading"><Check size={18} aria-hidden="true" /><strong>邀请已生成</strong></div><p className="muted-text">把下面链接发给 {createdInvite.inviteeNickname}，或让对方在当前浏览器模拟打开。</p><CopyBox value={inviteUrl} label="邀请链接" /><QrCode value={inviteUrl} label="共同监护邀请二维码（演示）" /><Button variant="secondary" onClick={() => openInviteLink(createdInvite.id)}><ArrowRight size={16} aria-hidden="true" />模拟受邀者打开</Button></div>}</Modal>
