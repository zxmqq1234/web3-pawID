...
import { Button, DemoBadge, Modal, TopBar, useToast } from '../../shared/ui';
...
const SERVICES: ReadonlyArray<ServiceDefinition> = [
  ...
  },
  {
    ...
    value: '让内容与 PawID 关联，形成可追溯的成长叙事；AI 生成能力仅作未来方向，本演示不调用真实 AI。',
  ...
  },
  {
    ...
    value: '通过 PawTag 指向受控的公开身份页，只分享必要资料并保留家庭隐私；寻回协作目前仅在本地演示。',
  ...
  },
  {
    ...
    value: '以宠物数字身份作为授权上下文，让宠主知道哪些资料被请求、用于什么场景；本演示不提供保险报价或理赔。',
  ...
  },
  {
    ...
    value: '以监护关系变更承接既有 PawID 与公开历史，让交接更清晰；本演示不代表任何真实救助机构或领养撮合。',
  ...
  },
  {
    ...
    value: '通过 PawTag 公开页承接必要线索，再由主人确认状态；本演示只在本地保存，不提供真实通知或定位。',
  ...
  return (
    <Modal open title={service.title} onClose={onClose} className="service-modal">
      <div className={`service-detail service-detail-${service.accent}`}>
        ...
        <section className="service-detail-value"><p className="service-detail-label"><PawPrint size={14} aria-hidden="true" /> Web3 价值（演示表达）</p><p>{service.value}</p></section>
        ...
        <p className="service-detail-disclaimer">本页为纯前端交互演示，服务方案待定。这里展示的是产品价值方向，不代表已完成真实认证、机构合作、链上交易、支付或订单。</p>
...
/** 服务生态页：本地筛选、详情和意向状态均来自演示 Store，不接真实服务。 */
export default function EcosystemPage(): JSX.Element {
  ...
  const toggleSelectedInterest = async (): Promise<void> => {
    ...
    try {
      ...
      showSuccess(wasInterested ? '已取消这项服务意向。' : '已记录感兴趣，意向保存在本地演示数据中。');
    } catch (error) {
  ...
  return (
    <div className="ecosystem-page">
      <TopBar messageSlot={<span className="ecosystem-top-note">纯前端演示 · 方案待定</span>}><EcosystemNavigation /></TopBar>
      <main>
        ...
        <section className="ecosystem-catalog" aria-labelledby="catalog-title">
          <div className="catalog-heading"><div><p className="section-eyebrow">SERVICE DIRECTIONS</p><h2 id="catalog-title">服务生态，从真实需求开始</h2></div><p>共 10 个 PRD 服务方向<br />先了解价值，再决定是否感兴趣</p></div>
