...
export default function InviteConfirmPage({ inviteId }: InviteConfirmPageProps): JSX.Element {
  ...
  return <main className="invite-public-page">
    <div className="invite-public-brand"><a href="#/" className="invite-back-link"><ArrowLeft size={16} aria-hidden="true" />PawID</a><span className="invite-mode-label">邀请确认 · 本机记录</span></div>
    <section className="invite-confirm-card" aria-labelledby="invite-confirm-title">
      ...
      <div className="invite-pet-summary"><PetAvatar pet={displayPet ?? pet} size="medium" showPresetLabel={false} /><div><h2>{pet.name}</h2><p>{pet.species === 'cat' ? '猫咪' : '狗狗'} · {pet.breed || '品种待补充'} · {pet.gender === 'female' ? '母' : pet.gender === 'male' ? '公' : '性别未知'}</p><p className="muted-text">{pet.introduction || '这是一份等待家人共同确认的陪伴记录。'}</p></div></div>
      ...
      {!canDecide && !isAccepted && !isRejected && <div className="invite-readonly-note"><ShieldCheck size={17} aria-hidden="true" /><span>{state.currentRole === 'owner' ? '主监护人不能接受自己发出的邀请，请切换为受邀者身份。' : '访客身份只读，只有受邀者可以接受或拒绝邀请。'}</span></div>}
